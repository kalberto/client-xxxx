	...
	angular.module('app').service('CHAMADAS', ['API', function (API) {
		...
		function paginateChamadas() {
			var starPage = void 0,
			    endPage = void 0;
			var x = 0;
			var number_pages = Math.ceil(data.pagination.total / data.pagination.limit);
			...
			if (data.pagination.page === 1) data.showing.primeiro = 1;else data.showing.primeiro = data.pagination.limit * (data.pagination.page - 1) + 1;
		...
		this.getChamadas = function () {
			data.chamadas = [];
			API.get('cliente/servicos/' + currentServico + '/call-detailed-time?page=' + data.pagination.page + '&limit=' + data.pagination.limit + '&search=' + data.pagination.query).then(function successCallback(response) {
		...
		this.getList = function () {
			...
			API.get('cliente/servicos/' + currentServico + '/calls?intervalo=' + data.date).then(function successCallback(response) {
		...
		this.setPage = function (navigate) {
			if (!data.carregando) {
				...
				if (navigate !== null) {
					if (navigate === 'ante') data.pagination.page -= 1;else if (navigate === 'prox') data.pagination.page += 1;
				}
				this.getChamadas();
			}
		};
		...
		this.popUp = function (ver) {
			if (ver === 1) {
				...
				if (!data.has) {
					data.carregando = true;
					API.get('cliente/servicos/' + currentServico + '/call-detailed-time').then(function successCallback(response) {
						data.chamadas = response.data.data;
						data.has = true;
						data.carregando = false;
						data.pagination.total = response.data.total;
