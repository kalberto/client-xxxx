(function () {
    ...
    function auth($scope, API, $window) {
        ...
        $scope.login = function () {
            API.post(routes.login, $scope.auth).then(function successCallback(response) {
                ...
                if (response.data && response.data.msg != undefined) $scope.popup.mensagem = response.data.msg;else $scope.popup.mensagem = "Tente novamente mais tarde";
                $scope.popup.botao = "Tente novamente";
            });
        };

        $scope.recuperar = function () {
            API.post(routes.recuperacao, $scope.auth).then(function successCallback(response) {
                $scope.popup.ativo = true;
        ...
        $scope.alterarSenha = function () {
            if ($scope.auth.password === $scope.auth.password_confirmation) {
                ...
                API.post(routes.alterar, $scope.auth).then(function successCallback(response) {
        ...
        $scope.fecharPopup = function () {
            if ($('#password').length > 0) $scope.auth.password = "";
            $scope.popup.titulo = "";
