    ...
    function contratos($scope, API) {
        ...
        $scope.paginateVigentes = function () {
            var starPage = void 0,
                endPage = void 0;
            var x = 0;
            var number_pages = Math.ceil($scope.total_vigentes / $scope.limit_vigentes);
            while ($scope.pages_vigentes.length > 0) {
                $scope.pages_vigentes.pop();
            }
            if (number_pages >= 1) {
                starPage = 1;
            ...
            if ($scope.page_vigentes == 1) $scope.showing_vigentes.primeiro = 1;else $scope.showing_vigentes.primeiro = $scope.limit_vigentes * ($scope.page_vigentes - 1) + 1;
        ...
        $scope.setPageVigentes = function (navigate) {
            if (!$scope.carregando_contratos_vigentes) {
                $scope.carregando_contratos_vigentes = true;
                if (navigate != null) {
                    if (navigate == 'ante') $scope.page_vigentes -= 1;else if (navigate == 'prox') $scope.page_vigentes += 1;
                }
        ...
        $scope.getContratosVigentes = function () {
            $scope.carregando_contratos_vigentes = true;
            API.get('cliente/contratos/vigentes?page=' + $scope.page_vigentes + '&limit=' + $scope.limit_vigentes).then(function successCallback(response) {
