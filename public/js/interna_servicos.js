(function () {
    ...
    function internaServicos($scope, API, CHAMADAS, LOGINS) {
        ...
        var colors = ["#008000", "#0000FF", "#9400D3", "#FF8000", "#CC0000", "#4B0082", "#800000", "#00B7A8", "#00491A", "#EA00FF", "#9AAD75", "#5A4706"];

        function initAmChartsVoz(response, categories) {

            var maxSize = colors.length;
            var graphs_values = [];
            var text = "<b>[[DATE]]</b><br><br><ul>";
            for (var _i = 0; _i < categories.length; _i++) {
                if (maxSize >= _i) {
                    graphs_values.push({
                        column: categories[_i],
                        fillColors: colors[_i]
                    });
                } else {
                    //TODO
                }
                text = text + "<li>" + categories[_i] + ": <b>[[" + categories[_i] + "]] " + $scope.short_unidade_grafico_voz + "</b> ([[" + (categories[_i] + "_percent") + "]]%)</li>";
            }
        ...
        function fixAmChartsVoz(response, categories) {
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                for (var _e = 0; _e < categories.length; _e++) {
                    if (tempSum !== 0) response[i][categories[_e] + '_percent'] = (parseFloat(response[i][categories[_e]]) / tempSum * 100).toFixed(2);else response[i][categories[_e] + '_percent'] = 0.00;
                }
            }
            return response;
        }
        ...
        function getGraficoConsumo() {
            ...
            API.get('cliente/servicos/' + $scope.servico.id + '/service-traffic').then(function successCallback(response) {
        ...
        function getGraficoVoz() {
            ...
            API.get('cliente/servicos/' + $scope.servico.id + '/called-traffic').then(function successCallback(response) {
        ...
        $scope.getPeriodo = function (start, end) {
            ...
            $scope.start = start;
            $scope.end = end;
            API.get('cliente/servicos/' + $scope.servico.id + '/service-traffic?start=' + (typeof start.toISOString === 'function' ? start.toISOString() : '') + '&end=' + (typeof end.toISOString === 'function' ? end.toISOString() : '')).then(function successCallback(response) {
                amcharts_listeners.consumo = [{
                    event: "init",
                    method: function method(e) {
                        // Por default, são exibidos dados correspondentes a 24hrs, com espaçamento de 05min.
        ...
        $scope.getPeriodoVoz = function (start, end, visao, custozero, consolidacao) {
            ...
            API.get('cliente/servicos/' + $scope.servico.id + '/called-traffic?start=' + (typeof start.toISOString === 'function' ? start.toISOString() : '') + '&end=' + (typeof end.toISOString == 'function' ? end.toISOString() : '') + '&visao=' + $scope.visao + '&custozero=' + $scope.custozero + '&consolidacao=' + $scope.consolidacao).then(function successCallback(response) {
                ...
                if (dados !== undefined && dados !== null && dados.length > 0) {
                    ...
                    var _dados = fixAmChartsVoz(response.data.data, response.data.categories);
                    if (amcharts_valueAxes.voz.length <= 0) initAmChartsVoz(_dados, response.data.categories);
