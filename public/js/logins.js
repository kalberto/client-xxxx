(function () {
	...
	angular.module('app').service('LOGINS', ['API', function (API) {
		...
		this.getLogins = function () {
			data.logins = [];
			API.get('cliente/servicos/' + currentServico + '/logins?page=' + data.pagination.page + '&limit=' + data.pagination.limit + '&search=' + data.pagination.query).then(function successCallback(response) {
