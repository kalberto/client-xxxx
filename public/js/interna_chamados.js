(function () {
    ...
    function internaChamados($scope, API, $timeout, $sce) {
        ...
        $scope.getHistorico = function () {
            API.get('cliente/chamados/' + id + '/nodes').then(function successCallback(response) {
        ...
        $scope.setRelatoAtivo = function (relato) {
            if ($scope.relatoAtivo == relato) $scope.relatoAtivo = null;else $scope.relatoAtivo = relato;
