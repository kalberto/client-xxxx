(function () {
    ...
    function faturas($scope, API) {
        ...
        $scope.getUltimasFaturas = function() {
            API.get('cliente/faturas/last?page='+$scope.selected.pageUlt+'&limit='+$scope.limitUlt).then(
        ...
        $scope.getFaturasAnteriores = function() {
            $scope.faturas_anteriores = [];
            let query = '?page='+$scope.selected.page+'&limit='+$scope.limit;
            if($scope.start && typeof $scope.start.toISOString === 'function')
                query += '&start='+ $scope.start.toISOString();
	        if($scope.end && typeof $scope.end.toISOString === 'function')
		        query += '&end='+ $scope.end.toISOString();
            API.get('cliente/faturas/list'+ query).then(
                function successCallback (response) {
        ...
        $scope.paginateUlt = function() {
            let starPage, endPage;
            let x = 0;
            let number_pages = Math.ceil($scope.totalUlt / $scope.limitUlt);
            while ($scope.pagesUlt.length > 0) { $scope.pagesUlt.pop(); }
            if(number_pages >= 1) {
                starPage = 1;
                endPage = number_pages;
                for (let i = starPage; i <= endPage; i++){
            ...
            if ($scope.selected.pageUlt == 1)
                $scope.showingUlt.primeiro = 1;
            else
                $scope.showingUlt.primeiro = ($scope.limitUlt * ($scope.selected.pageUlt-1)) + 1;
        ...
        $scope.fecharPopup = function(id) {
            switch(id) {
                case 'nao_implementado': $scope.popup_NaoImplementado = false; break;
            }
        };
