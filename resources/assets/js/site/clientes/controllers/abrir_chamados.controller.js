(function () {
    ...
    function chamados($scope, API) {
        ...
        $scope.getAssuntos = function () {
            ...
            $scope.carregando_chamados = true;
            API.get('cliente/contato/fields').then(
                function successCallback(response) {
                    $scope.assuntos = Object.keys(response.data.assuntos);
                    $scope.all = response.data.assuntos;
                    $scope.carregando_assuntos = false;
                    if($scope.carregando_email == false && $scope.carregando_services == false)
        ...
        $scope.getServices = function (url_produto) {
            ...
            $scope.carregando_chamados = true;
            API.get('cliente/chamados/services?url_produto='+url_produto).then(
                function successCallback(response) {
                    $scope.servicos = response.data;
                    if($scope.servicos.length == 1)
                        $scope.form.servico = $scope.servicos[0].id;
                    $scope.carregando_services = false;
                    if($scope.carregando_assuntos == false && $scope.carregando_email == false)
                        $scope.carregando_chamados = false;
        ...
        $scope.getLastChamados = function(){
            $scope.carregando = true;
            API.get('cliente/chamados/last').then(
                function successCallback(response) {
        ...
        $scope.abrirChamado = function (url_produto) {
            ...
            if(!$scope.already_load.assuntos)
                $scope.getAssuntos();
        ...
        $scope.postContato = function () {
            ...
            API.post('cliente/chamado', $scope.form).then(
                function successCallback(response) {
                    $scope.form = {
                        email: '',
                        assunto: '',
