(function () {
    ...
    function perfil($scope, API, $http, Upload) {
        ...
        $scope.editarEndereco = function(){
            ...
            API.post('cliente/perfil/editar',$scope.usuario).then(
        ...
        $scope.editarSenha = function() {
            ...
            API.post('cliente/perfil/senha',$scope.auth).then(
                function successCallback (response) {
                    $scope.carregando_senha = false;
        ...
        $scope.autocompletePorCep = function () {
            if($scope.usuario.endereco.cep.length >= 8){
                $http.get('https://viacep.com.br/ws/'+$scope.usuario.endereco.cep+'/json/')
            ...
            if(file){
                Upload.upload({
                    ...
                    function successCallback(response) {
                        ...
                        API.get('cliente/perfil/media/'+$scope.usuario.media_id).then(
                            function successCallback(response) {
