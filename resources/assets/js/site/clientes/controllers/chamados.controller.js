(function () {
    ...
    function chamados($scope, API) {
        ...
        $scope.search = function(q) {
            ...
            API.get('cliente/chamados/load?search=' + q).then(
        ...
        $scope.getChamados = function(){
            $scope.carregando_ativos = true;
            API.get('cliente/chamados/load?page='+$scope.page_ativos + '&limit=' + $scope.limit_ativos).then(
                function successCallback(response) {
        ...
        $scope.paginateAtivos = function() {
            ...
            let x = 0;
            let number_pages = Math.ceil($scope.total_ativos / $scope.limit_ativos);
            while ($scope.pages_ativos.length > 0) { $scope.pages_ativos.pop(); }
            if(number_pages >= 1) {
                starPage = 1;
            ...
            if ($scope.page_ativos == 1)
                $scope.showing_ativos.primeiro = 1;
            else
                $scope.showing_ativos.primeiro = ($scope.limit_ativos * ($scope.page_ativos-1)) + 1;
        ...
        $scope.getChamadosResolvidos = function () {
            $scope.chamados = [];
            $scope.carregando_resolvidos = true;
            API.get('cliente/chamados/resolved?page='+$scope.page_resolvidos + '&limit=' + $scope.limit_resolvidos).then(
                function successCallback(response) {
        ...
        $scope.paginateResolvidos = function() {
            let starPage, endPage;
            let x = 0;
            let number_pages = Math.ceil($scope.total_resolvidos / $scope.limit_resolvidos);
            while ($scope.pages_resolvidos.length > 0) { $scope.pages_resolvidos.pop(); }
            if(number_pages >= 1) {
                starPage = 1;
                endPage = number_pages;
                for (let i = starPage; i <= endPage; i++){
            ...
            if ($scope.page_resolvidos == 1)
                $scope.showing_resolvidos.primeiro = 1;
            else
                $scope.showing_resolvidos.primeiro = ($scope.limit_resolvidos * ($scope.page_resolvidos-1)) + 1;
